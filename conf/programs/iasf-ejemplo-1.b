...
agent ReactiveControl
{
	...
	blender
	{
		...
		rules
		{
			...
			if (leftd is CLOSE)					avoidL is HIGH;
			if (front is CLOSE)					avoidF is HIGH;
			if (rightd is CLOSE)				avoidR is HIGH;

			if (leftd is NEAR) 					avoidL is HALFH;
			if (front is NEAR)					avoidF is HALFH;
			if (rightd is NEAR) 				avoidR is HALFH;

			if (leftd is MED) 					avoidL is HALFL;
			if (front is MED)					avoidF is HALFL;
			if (rightd is MED) 					avoidR is HALFL;
		}
	}
}
