...
agent ReactiveControl
{
	...
	behaviour toGoal priority 0.7
	{
		...
		rules
		{
			if (diff is GP)			vrot is TTL, vlin is SLOW;
			if (diff is GSP)		vrot is TL, vlin is SMEDIUM;		// TSL
			if (diff is GZ)			vrot is TC, vlin is SFULL;
			if (diff is GSN)		vrot is TR, vlin is SMEDIUM;		// TSR
			if (diff is GN)			vrot is TTR, vlin is SLOW;
		}
	}
