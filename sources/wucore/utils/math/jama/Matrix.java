package wucore.utils.math.jama;
...
public class Matrix implements Serializable 
{
   ...
   Public Methods
	...
	/** Set a complete diagonal.
   @param s    A(i,i).
   @exception  ArrayIndexOutOfBoundsException
	 */

	public Matrix diagonal (double s) {
		for (int i = 0; i < m; i++)
			for (int j = 0; j < n; j++)
				if (i == j) 
					A[i][j] = s;
   ...
   @exception  ArrayIndexOutOfBoundsException
	...
	public void getMatrix (PointH point)
	{
		if ((m != 4) || (n != 1))
			throw new IllegalArgumentException ("Matrix MUST be 4 x 1");

		point.set (A[0][0], A[1][0], A[2][0], A[3][0]);
	}
   ...
   @exception  ArrayIndexOutOfBoundsException Submatrix indices
	...
	public void getRow (int i, Point3 point)
	{
		if (n != 3)
			throw new IllegalArgumentException ("Matrix MUST be m x 3");

		point.set (A[i][0], A[i][1], A[i][2]);
	}
   ...
   @exception  ArrayIndexOutOfBoundsException
	...
	/** Increment a single element.
   @param i    Row index.
   @param j    Column index.
   @param s    amount to increase.
   @exception  ArrayIndexOutOfBoundsException
	 */
	...
	public void setMatrix (PointH point)
	{
		if ((m != 4) || (n != 1))
			throw new IllegalArgumentException ("Matrix MUST be 4 x 1");
   ...
   @exception  ArrayIndexOutOfBoundsException Matrix size
	 */
	...
	public void setMatrix (Matrix X) {
		try {
			...
			throw new ArrayIndexOutOfBoundsException("Matrix size");
		}
	}
   ...
   @exception  ArrayIndexOutOfBoundsException Submatrix indices
	...
	public void setRow (int i, Point3 point)
	{
		if (n != 3)
			throw new IllegalArgumentException ("Matrix MUST be m x 3");
   ...
   @return    maximum singular value.
	...
	public double norm2 () {
		return (new SVDecomposition(this).norm2());
	}
   ...
   @exception  IllegalArgumentException Matrix inner dimensions must agree.
	...
	public Matrix jacobian ()
	{
		...
		for (count=0; count < ntot; count++) //ntot is # of sweeps
			for(p=0; p < m-1; p++) //sweep through aa matrix
				for(q=p+1; q < m; q++) //zero elements via Jacobi transformation			   
					if(aa.get (p,q) != 0.)
					{
						a=aa.get (p,p); //from here, its same as 2x2 case as in lecture
						b=aa.get (p,q);
						d=aa.get (q,q);

						theta=(d-a)/(2.*b);
						t1=-theta+Math.sqrt(theta*theta+1.);
						t2=-theta-Math.sqrt(theta*theta+1.);
						...
						s=c*t;
						h=t*b;
						aa.set (p,p, aa.get (p,p)-h);
						aa.set (q,q, aa.get (q,q)+h);
						aa.set (p,q, 0.0);
						aa.set (q,p, 0.0);
						tau=s/(1.+c);

						for(r=0; r < p; r++){
							g1=aa.get (p,r);						g2=aa.get (q,r);
							aa.set (p,r, g1-s*(g2+tau*g1));		aa.set (q,r, g2+s*(g1-tau*g2));
							aa.set (r,p, aa.get (p,r));			aa.set (r,q, aa.get (q,r));
						}
						for(r=p+1; r < q; r++){
							g1=aa.get (p,r);						g2=aa.get (q,r);
							aa.set (p,r, g1-s*(g2+tau*g1));		aa.set (q,r, g2+s*(g1-tau*g2));
   ...
   @return     solution if A is square, least squares solution otherwise
	...
	/** Solves the set of n linear equations A.X=B
  	...
  	@param b is the vector B (input)

  	@return is the vector X (output)
	 */
	public double[] solve (double[] b)
	{
		Matrix B = new Matrix (b, b.length);
		return solve (B).getRowPackedCopy ();
	}
   ...
   Private Methods
	 ...
	 * @exception java.lang.IllegalArgumentException The exception description.
	 */
	public Matrix vectorProduct(Matrix B) throws java.lang.IllegalArgumentException
	{
