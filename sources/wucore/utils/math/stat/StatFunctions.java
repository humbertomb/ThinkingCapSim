...
public final class StatFunctions {
  ...
  public static double pValue_pnorm(double prob){
  		//David Herrero Pérez 2002.
	double pnorm1 = 0.0,pnorm2,pnormm;
	double epsi = 0.0001, lon;
	if(prob == 0.5){
		return 0.0;
	} else if (prob > 0.5){
		pnorm2 = 1.0;
		while (prob > pnorm(pnorm2,false)){
			pnorm1 = pnorm2;
			pnorm2++;
		}
		lon = pnorm2-pnorm1;	
		while (lon > epsi){
			pnormm = 0.5*(pnorm1+pnorm2);
			if (prob > pnorm(pnormm,false)){
				pnorm1=pnormm;
			} else {
				pnorm2=pnormm;
			}
  ...
  public static double pValue_pt(double prob,double df){
  		//David Herrero Pérez 2002.
	double t1 = 0.0,t2,tm;
	double epsi = 0.0001, lon;
	if(prob == 0.5){
