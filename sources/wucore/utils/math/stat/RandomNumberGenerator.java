...
public class RandomNumberGenerator implements java.io.Serializable 
{
	...
	public synchronized int nextInt(int n) {
		...
		if ((n & -n) == n) { 
			// i.e., n is a power of 2 return (int)((n * (long)next(31)) >> 31); 
		}
		int bits, val; 
		do {
			bits = next(31); 
...
//	E(X)=(b-a)/2 ; Var(X)=(b-a)^2/12
	...
	// generate N(m,s2)
	//	E(X)=m ; Var(X)=s2
	private double lastNormLow;
	private double lastNormHigh;
	
	public double nextGaussian (double m, double s2, double normLow, double normHigh) 
	{
		...
		double fac,r,v1,v2,mm,a,result;
		do {
			if  (!haveNextGaussian ||((normLow!=lastNormLow)||(normHigh!=lastNormHigh))) {
				/* code to generate out in the tails of the normal*/
				lastNormLow = normLow;
				lastNormHigh = normHigh;
				if((normHigh<0)||(normLow>0)) {
					mm = Math.max(normLow,-normHigh);
					a = (double)Math.exp((double)(-0.5*(mm*mm)));
					do {
						v1 = nextUniform (-a, a);
						v2 = nextUniform (-a, a);
						r = v1*v1+v2*v2;
					...
					a = 1.0;
					do {
						v1 = nextUniform (-1.0, 1.0);
						v2 = nextUniform (-1.0, 1.0);
						r = v1*v1+v2*v2;
					} while (r >= a);
				}
			...
			if(((normHigh<0)&&(result>0))||((normLow>0)&&(result<0)))
				result = -result;
		} while((result<normLow)||(result>normHigh));
		
		return m+result*Math.sqrt (s2);
	}
