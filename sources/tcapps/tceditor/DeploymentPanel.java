...
public class DeploymentPanel extends JPanel implements ArchCanvas.Listener
{
	...
	static private final String[][]	EVENT_HELP	=
	{
		...
		{ "PALLET_CTRL",	"What a forklift is to do with a pallet: the pallet by its name, where it stands, whether it is free, free with a box on it or loaded, and whether it is to be added, taken, moved or moved and changed. It travels the global space, and a forklift acts on the ones addressed to it. Nothing of the development writes it: it is written from outside, by whoever runs the warehouse, and is waiting for the simulator to be put together with a simulator of a production line. Of all it carries, a virtual forklift today heeds only ADD and DEL of a pallet of its own, which is what puts the pallet on its forks or takes it off." },
